...
test.describe('Navigation', () => {
  test(qase(1, 'navigates to registration page via login page link'), { tag: '@smoke' }, async ({ registerPage, page }) => {
  ...
  test(qase(2, 'shows all 4 progress bar steps'), { tag: '@smoke' }, async ({ registerPage }) => {
...
test.describe('Registration form — field validation', () => {
  ...
  test(qase(3, 'empty email shows validation error'), async ({ registerPage }) => {
  ...
  test(qase(4, 'empty password shows validation error'), async ({ registerPage }) => {
  ...
  test(qase(5, 'invalid email format shows validation error'), async ({ registerPage }) => {
  ...
  test(qase(6, 'excessively long email is rejected with well-formed email error'), async ({ registerPage }) => {
    const longEmail = 'longlonglonglonglonglonglonglonglonglonglonglonglonglonglonglonglonglonglonglonglonglonglonglong@test.com';
    ...
    await expect(registerPage.serverErrorBanner).toContainText('must be a well-formed email address');
    await expect(registerPage.activationPopup).not.toBeVisible();
  });

  test(qase(7, 'unchecked terms checkbox blocks submission'), async ({ registerPage }) => {
    await registerPage.fillEmail(uniqueEmail());
    await registerPage.fillPassword(VALID_PASSWORD);
    await registerPage.clickRegister();
  ...
  test(qase(8, 'already-registered email shows server error'), { tag: '@smoke' }, async ({ registerPage }) => {
    await registerPage.register('test@test.com', VALID_PASSWORD);

    await expect(registerPage.serverErrorBanner).toBeVisible();
    await expect(registerPage.serverErrorBanner).toContainText('User with this email already exist');
    await expect(registerPage.activationPopup).not.toBeVisible();
  });

  test.skip(qase(22, 'password shorter than minimum length is rejected'), async () => {
    // TODO: minimum length not documented; needs exploration to confirm the rule.
  });

  test.skip(qase(23, 'multiple invalid email format variants are each rejected'), async () => {
    // TODO: "@domain.com", "user@", "user@domain", "user@@domain.com"
  });
});
...
test.describe('Password field', () => {
  test(qase(9, 'toggles visibility when eye icon is clicked'), async ({ registerPage }) => {
    await registerPage.goto();
    await registerPage.fillPassword(VALID_PASSWORD);
  ...
  test(qase(10, 'hides password again when eye icon is clicked a second time'), async ({ registerPage }) => {
    await registerPage.goto();
    await registerPage.fillPassword(VALID_PASSWORD);
...
test.describe('Successful registration', () => {
  ...
  test(qase(11, 'shows activation popup with the registered email'), { tag: '@smoke' }, async ({ registerPage }) => {
    await registerPage.verifyActivationPopupVisible();
    await expect(registerPage.activationEmailDisplay).toHaveText(email);
  });

  test(qase(12, 'activation popup contains all expected UI elements'), async ({ registerPage }) => {
    await registerPage.verifyActivationPopupVisible();
...
test.describe('Verification code popup', () => {
  ...
  test(qase(13, 'wrong code shows error and popup displays the registered email'), async ({ registerPage }) => {
    await expect(registerPage.activationEmailDisplay).toHaveText(email);
    ...
    await expect(registerPage.activationCodeError).toContainText('Error with code. Please check.');
  });

  test(qase(14, 'resend link shows success message'), async ({ registerPage }) => {
    await registerPage.activationResendLink.click();

    await expect(registerPage.activationResendSuccess).toBeVisible();
    await expect(registerPage.activationResendSuccess).toContainText('A new verification code was sent.');
  });

  test(qase(15, 'partial code entry does not trigger error prematurely'), async ({ registerPage }) => {
    const inputs = registerPage.activationCodeInputs;
    for (let i = 0; i < 3; i++) {
      await inputs.nth(i).fill(String(i + 1));
    }
...
test.describe('Update email popup', () => {
  ...
  test(qase(16, 'valid new email updates the verification screen'), async ({ registerPage }) => {
  ...
  test(qase(17, 'cancel keeps the original email unchanged'), async ({ registerPage }) => {
    await registerPage.updateEmailInput.fill(uniqueEmail());
    await registerPage.cancelEmailButton.click();

    await registerPage.verifyActivationPopupVisible();
    await expect(registerPage.activationEmailDisplay).toHaveText(originalEmail);
  });

  test(qase(18, 'invalid email shows "Invalid e-mail" error and icon'), async ({ registerPage }) => {
  ...
  test(qase(19, 'empty email shows "Required field" error and icon'), async ({ registerPage }) => {
    await registerPage.updateEmailButton.click();
  ...
  test(qase(20, 'same email as original shows server error'), async ({ registerPage }) => {
  ...
  test(qase(21, 'already-registered email shows server error'), async ({ registerPage }) => {
