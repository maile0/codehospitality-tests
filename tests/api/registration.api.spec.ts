...
async function registerUser(request: APIRequestContext, email = uniqueEmail()) {
  const res = await request.post(`${ENDPOINTS.register}?unauthId=${crypto.randomUUID()}`, {
    data: { email, password: VALID_PASSWORD, termsAndConditionsConsent: true, promoCode: '' },
  });
...
test.describe('POST /api/web/members/register', () => {
  ...
  test('missing email returns 400', async ({ request }) => {
    const res = await request.post(`${ENDPOINTS.register}?unauthId=${crypto.randomUUID()}`, {
      data: { password: VALID_PASSWORD, termsAndConditionsConsent: true, promoCode: '' },
    });
  ...
  test('missing password returns 400', async ({ request }) => {
    const res = await request.post(`${ENDPOINTS.register}?unauthId=${crypto.randomUUID()}`, {
      data: { email: uniqueEmail(), termsAndConditionsConsent: true, promoCode: '' },
    });
  ...
  test('invalid email format returns 400', async ({ request }) => {
    const res = await request.post(`${ENDPOINTS.register}?unauthId=${crypto.randomUUID()}`, {
      data: { email: 'not-an-email', password: VALID_PASSWORD, termsAndConditionsConsent: true, promoCode: '' },
    });
  ...
  test('terms not accepted returns 400', async ({ request }) => {
    const res = await request.post(`${ENDPOINTS.register}?unauthId=${crypto.randomUUID()}`, {
      data: { email: uniqueEmail(), password: VALID_PASSWORD, termsAndConditionsConsent: false, promoCode: '' },
    });
